...
def upscale2d_conv2d(x, fmaps, kernel, gain=np.sqrt(2), use_wscale=False, trainable=None):
    ...
    w = get_weight([kernel, kernel, fmaps, x.shape[1].value], gain=gain, use_wscale=use_wscale,
                   fan_in=(kernel ** 2) * x.shape[1].value, trainable=trainable)
                ...
                if res == 2:  # 4x4
                    ...
                    with tf.variable_scope('Dense', reuse=tf.AUTO_REUSE) as scope_dense:
                        with absolute_name_scope(scope_dense.name):
                            x = dense(x, fmaps=nf(res - 1) * 16, gain=np.sqrt(2) / 4, use_wscale=use_wscale,
                                      trainable=False)  # override gain to match the original Theano implementation
                            x = tf.reshape(x, [-1, nf(res - 1), 4, 4])
                    ...
                    with tf.variable_scope('Conv', reuse=tf.AUTO_REUSE) as scope_conv:
                        with absolute_name_scope(scope_conv.name):
                            x = PN(act(apply_bias(
                                conv2d(x, fmaps=nf(res - 1), kernel=3, use_wscale=use_wscale, trainable=False),
                                trainable=False)))
                else:  # 8x8 and up
                    if fused_scale:
                        with tf.variable_scope('Conv0_up', reuse=tf.AUTO_REUSE) as scope_conv0up:
                            with absolute_name_scope(scope_conv0up.name):
                                x = PN(act(apply_bias(
                                    upscale2d_conv2d(x, fmaps=nf(res - 1), kernel=3, use_wscale=use_wscale,
    ...
    def torgb(x, res):  # res = 2..resolution_log2
        ...
        with tf.variable_scope('ToRGB_lod%d' % lod, reuse=tf.AUTO_REUSE) as scope_torgb:
            with absolute_name_scope(scope_torgb.name):
                return apply_bias(
                    conv2d(x, fmaps=num_channels, kernel=1, gain=1, use_wscale=use_wscale, trainable=False),
                    trainable=False)
    ...
    def fromrgb(x, res):  # res = 2..resolution_log2
        with tf.variable_scope('FromRGB_lod%d' % (resolution_log2 - res)) as rgb_scope:
            with absolute_name_scope(rgb_scope.name):
                return act(apply_bias(conv2d(x, fmaps=nf(res - 1), kernel=1, use_wscale=use_wscale, trainable=False),
                                      trainable=False))
                    ...
                    if fused_scale:
                        with tf.variable_scope('Conv1_down') as conv1_down_scope:
                            with absolute_name_scope(conv1_down_scope.name):
                                x = act(apply_bias(
                                    conv2d_downscale2d(x, fmaps=nf(res - 2), kernel=3, use_wscale=use_wscale,
                                                       trainable=False), trainable=False))
                    else:
