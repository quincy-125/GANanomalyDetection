...
class TrainingSchedule:
	# noinspection PyDefaultArgument,PyDefaultArgument,PyDefaultArgument,PyDefaultArgument,PyDefaultArgument
	def __init__(
		self,
		cur_nimg,
		training_set,
		...
		#print('self.minibatch: {}'.format(self.minibatch))
...
def train_progressive_gan(
	...
	for gpu in range(config.num_gpus):
		with tf.compat.v1.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
			...
			lod_assign_ops = [tf.compat.v1.assign(G_gpu.find_var('lod'), lod_in), tf.compat.v1.assign(D_gpu.find_var('lod'), lod_in), tf.compat.v1.assign(E_gpu.find_var('lod'), lod_in)]
			...
			with tf.compat.v1.name_scope('E_loss'), tf.control_dependencies(lod_assign_ops):
			...
			E_opt.register_gradients(tf.reduce_mean(input_tensor=E_loss), E_gpu.trainables)
