...
def train_progressive_gan(
    ...
    with tf.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            E = tfutil.Network('E', num_channels=training_set.shape[0], resolution=training_set.shape[1],
                               label_size=training_set.label_size, **config.E)
            Gs = G.clone('Gs')
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, E=E_gpu, opt=G_opt, training_set=training_set,
            ...
            with tf.name_scope('E_loss'), tf.control_dependencies(lod_assign_ops):
                E_loss = tfutil.call_func_by_name(G=G_gpu, E=E_gpu, opt=E_opt, training_set=training_set,
                                                  minibatch_size=minibatch_split, reals=reals_gpu, labels=labels_gpu,
                                                  **config.E_loss)
    ...
    misc.save_image_grid(grid_fakes, os.path.join(result_subdir, 'fakes%06d.png' % 0), drange=drange_net,
                         grid_size=grid_size)
    misc.save_image_grid(grid_fakes_reconstructed, os.path.join(result_subdir, 'reconstructed_fakes%06d.png' % 0),
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if cur_tick % image_snapshot_ticks == 0 or done:
                grid_fakes = Gs.run(grid_latents, grid_labels, minibatch_size=sched.minibatch // config.num_gpus)
                latents_out = E.run(grid_fakes, minibatch_size=sched.minibatch // config.num_gpus)
                grid_fakes_reconstructed = Gs.run(latents_out, grid_labels,
                ...
                misc.save_image_grid(grid_fakes, os.path.join(result_subdir, 'fakes%06d.png' % (cur_nimg // 1000)),
                                     drange=drange_net, grid_size=grid_size)
                misc.save_image_grid(grid_fakes_reconstructed,
                                     os.path.join(result_subdir, 'fakes_reconstructed%06d.png' % (cur_nimg // 1000)),
                                     drange=drange_net, grid_size=grid_size)
