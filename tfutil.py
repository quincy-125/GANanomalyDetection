...
def _create_autosummary_var(name, value_expr):
	...
	update_op = tf.cond(pred=tf.is_variable_initialized(var), true_fn=lambda: tf.assign_add(var, v),
	                    false_fn=lambda: tf.assign(var, v))
				...
				with tf.name_scope('ApplyGrads%d' % dev_idx), tf.device(dev):
					...
					with tf.name_scope('CheckOverflow'):
						grad_ok = tf.reduce_all(
							input_tensor=tf.stack([tf.reduce_all(input_tensor=tf.math.is_finite(g)) for g, v in grads]))
					...
					with tf.name_scope('UpdateWeights'):
						...
						if not self.use_loss_scaling:
							ops.append(
								tf.cond(pred=grad_ok, true_fn=lambda: opt.apply_gradients(grads), false_fn=tf.no_op))
						else:
...
class Network:
	...
	def _init_graph(self):
			...
			with absolute_name_scope(self.scope):  # ignore surrounding name_scope
				with tf.control_dependencies(None):  # ignore surrounding control_dependencies
					if self.name == 'G_anomaly':
						self.ano_z = tf.get_variable('ano_z', shape=[8, 1024], dtype=tf.float32,
						...
						if "images_in" in self.static_kwargs:
							...
							self.input_templates = [tf.placeholder(tf.float32, name=name) for name in
							                        self.input_names]
						out_expr = self._build_func(*self.input_templates, is_template_graph=True, **self.static_kwargs)
		...
		self.vars = OrderedDict(
			[(self.get_var_localname(var), var) for var in tf.global_variables(self.scope + '/')])
				...
				for gpu in range(num_gpus):
					with tf.device('/gpu:%d' % gpu):
						...
						if out_shrink > 1:
							...
							out_expr = [tf.nn.avg_pool2d(input=x, ksize=ksize, strides=ksize, padding='VALID',
...
class AnomalyDetectorEncoder(object):
	...
	def find_closest_match(self, test_data, test_data_name, test_batch_size=8):
		...
		test_data_tensor = tf.convert_to_tensor(value=np.array(test_data))
		...
		res_loss = tf.reduce_sum(input_tensor=tf.reduce_sum(
			input_tensor=tf.reduce_sum(input_tensor=tf.abs(tf.subtract(test_data_tensor, closest_matches)), axis=1),
			axis=1), axis=1)
		...
		dis_loss = tf.reduce_sum(input_tensor=tf.reduce_sum(
			input_tensor=tf.reduce_sum(input_tensor=tf.abs(tf.subtract(dis_f_z, dis_f_input)), axis=1), axis=1), axis=1)
		...
		for i in range(samples.shape[0]):
			...
			filename = ['AD_' + test_data_name[i].split("\\")[-1],
			            'AD_error_' + str(sess.run(ano_score)[i]) + '_' + str(sess.run(res_loss)[i]) + '_' +
			            test_data_name[i].split("\\")[-1],
