...
def create_cifar10_anomaly_detection(tfrecord_dir, cifar10_dir, anomaly_class, percentage):
    ...
    for i, ii in enumerate(I):
        ...
        while len(filename) < 10:
            filename = '0' + filename
        cv2.imwrite(os.path.join(tfrecord_dir, "train", filename),
                    cv2.cvtColor(np.moveaxis(images[ii, :, :, :], 0, -1), cv2.COLOR_RGB2BGR))
    # im_rgb = images[ii,:,:,:];
    # im_gray = 0.2989*im_rgb[0,:,:] + 0.5870*im_rgb[1,:,:] + 0.1140*im_rgb[2,:,:];
    ...
    for i, ii in enumerate(I_normal):
        ...
        while len(filename) < 10:
            filename = '0' + filename
        cv2.imwrite(os.path.join(tfrecord_dir, "validation", "cell", filename),
                    cv2.cvtColor(np.moveaxis(test_images[ii, :, :, :], 0, -1), cv2.COLOR_RGB2BGR))
    ...
    for i, ii in enumerate(I_anomaly):
        ...
        cv2.imwrite(os.path.join(tfrecord_dir, "validation", "junk", filename),
                    cv2.cvtColor(np.moveaxis(test_images[ii, :, :, :], 0, -1), cv2.COLOR_RGB2BGR))
...
def execute_cmdline(argv):
    ...
    p.add_argument('tfrecord_dir', help='New dataset directory to be created')
    p.add_argument('image_dir', help='Directory containing the images')
    p.add_argument('included_anomaly_dir',
                   help='Directory containing list of anomalies that should be included in the training data')
    p.add_argument('--shuffle', help='Randomize image order (default: 1)', type=int, default=1)
